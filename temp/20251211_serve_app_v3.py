...
if __name__ == "__main__":
    ...
    ray.init()
    # serve.start는 Ray 2.4 이후 권장하지 않고, serve.run 사용을 권장
    # http_options는 serve.run() 대신 ray.init 옵션으로 설정 가능하지만 여기서는 간단하게 기본 포트 사용
    serve.shutdown()  # 혹시 이전 실행이 남아있다면 초기화
    ...
    # ModelService를 바인딩한 후 route_prefix를 serve.run에서 지정
    model_service = ModelService.bind()
    serve.run(model_service, name="model_service", route_prefix="/")